class AutopsiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @autopsy.save
        format.html { redirect_to @autopsy, notice: 'Autopsy was successfully created.' }
        format.json { render :show, status: :created, location: @autopsy }
      else
        format.html { render :new }
        format.json { render json: @autopsy.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def update
    respond_to do |format|
      if @autopsy.update(autopsy_params)
        format.html { redirect_to @autopsy, notice: 'Autopsy was successfully updated.' }
  ...
  def destroy
    @autopsy.destroy
    respond_to do |format|
      format.html { redirect_to autopsies_url, notice: 'Autopsy was successfully destroyed.' }
  ...
  private
    ...
    def autopsy_params
      params.require(:autopsy).permit(:completed, :number, :conclusion, :suspect_id, :victim_id, :place_id, :autopsy_date, :starting_time, :ending_time, :police_station_id, :police_inspector_id, :court_id, :judge_id)
    end
end
